...
// Функция возвращает значение реквизита "Наименование".
// 
// Параметры:
//  Документ - ДокументСсылка.ПоступлениеТоваров, ДокументСсылка.ПеремещениеТоваров, ДокументСсылка.ПересчетТоваров, ДокументСсылка.РеализацияТоваров - Документ
//  Штрихкод - Строка
...
// Возвращаемое значение:
//  Строка
Функция НаименованиеПоШтрихкоду(Знач Документ, Знач Штрихкод) Экспорт

    Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
                          |	ВсеШтрихкоды.Номенклатура.Наименование КАК Наименование
                          |ИЗ
                          |	РегистрСведений.ШтрихкодыТоваров КАК ВсеШтрихкоды
                          |ГДЕ
                          |	ВсеШтрихкоды.Документ = &Документ
    ...
    Если Не Выборка.Следующий() Тогда
        ...
        ВызватьИсключение ("Не удалось получить наименование по штрихкоду");

    КонецЕсли;
    
    //@skip-check property-return-type
    Наименование = Выборка.Наименование; // Строка
