...
Асинх Процедура ОбработатьШтрихкодАсинх(Штрихкод)
    ...
    Если ХарактеристикиНоменклатуры.Количество() > 0 Тогда
		...
		ХарактеристикиНоменклатурыПоМестуХранения = Новый СписокЗначений;
		
		Для Каждого Элемент Из ХарактеристикиНоменклатуры Цикл
			Если Не СтрНачинаетсяС(Строка(Элемент.Представление), "*") Тогда
				ХарактеристикиНоменклатурыПоМестуХранения.Добавить(Элемент.Значение, Элемент.Представление);
			КонецЕсли;
		КонецЦикла;
		
		Если ХарактеристикиНоменклатурыПоМестуХранения.Количество() = 0 Тогда
        	ПоказатьОповещение("Для данной номенклатуры не задана цена по месту хранения", ПредставлениеНоменклатуры);
			Возврат;
		Иначе
			ВыбраннаяХарактеристика = Ждать ХарактеристикиНоменклатурыПоМестуХранения.ВыбратьЭлементАсинх(ПредставлениеНоменклатуры);
			Если ВыбраннаяХарактеристика = Неопределено Тогда
		        // Пользователь отказался от выбора
		        Возврат;
		    КонецЕсли;
...
&НаСервереБезКонтекста
Функция СуммаДокумента(ПересчетТоваров)
	
	Возврат РегистрыСведений.УдалитьШтрихкодыТоваровСХарактеристиками.СуммаДокумента(ПересчетТоваров);
...
Процедура ОбновитьФорму(ОбновитьСписокТоваров = Истина)
	...
	ФормаСуммаДокумента = СуммаДокумента(ФормаДокумент);
